...
export function PipelineBoard() {
    ...
    try {
      ...
      if (jobs.length === 0) {
        ...
        if (jobsRes.data.success) {
          companyJobs = jobsRes.data.data.filter(
            (j: any) => j.company_id === profile?.id,
          );
          setJobs(companyJobs);
        }
      }
      ...
      setAllApplicants((prev) =>
        prev.map((a) =>
          a.application_id === appId
            ? {
                ...a,
                status: numericStageId.toString(),
  ...
  const currentApplicants = useMemo(() => {
    ...
    list = list.map((a) => {
      if (selectedJobId !== "ALL" && customStages.length > 0) {
        ...
        if (!stg && (a.status === "SELECTED" || a.status === "SHORTLISTED"))
  ...
  const handleExportCSV = (data: any[]) => {
    ...
    link.setAttribute(
      "download",
      `talentbridge_export_${selectedStageView || "all"}.csv`,
    );
                    ...
                    <tbody className="divide-y divide-slate-100 bg-white">
                      {currentPageApplicants.map((cand, idx) => {
                        ...
                        const isContactedLocal =
                          !!contactedCandidates[cand.application_id];
