...
function ProfileCompactCard({ profile }: { profile: any }) {
  ...
  return (
    <div className="h-full bg-white border border-slate-100 rounded-3xl p-5 text-center relative overflow-hidden group shadow-md shadow-slate-200/40 flex flex-col items-center justify-center hover:shadow-lg transition-all duration-300">
       <div className="w-16 h-16 bg-gradient-to-tr from-indigo-100 to-blue-50 rounded-2xl border-4 border-white shadow-md flex items-center justify-center mb-3 relative z-10 overflow-hidden shrink-0">
          {profile?.profile_photo_url ? (
            <img src={profile.profile_photo_url} className="w-full h-full object-cover" />
          ) : (
            <FileUser size={32} className="text-indigo-400" />
          )}
       </div>
       <h2 className="text-base font-black text-slate-900 uppercase tracking-tight mb-1 truncate w-full"><DynamicText text={profile?.full_name || t('my_profile')} /></h2>
       <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 px-2 truncate w-full">
       ...
       <div className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 mb-4 text-left">
          <div className="flex justify-between items-end mb-1.5">
             <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">{t('profile_strength')}</span>
             <span className="text-xs font-black text-indigo-600">{score}%</span>
          </div>
          <div className="h-1.2 w-full bg-slate-200 rounded-full overflow-hidden">
             <div className="h-full bg-indigo-600 rounded-full" style={{ width: `${score}%` }} />
          </div>
       </div>

       <Link to="/profile" className="w-full py-3 bg-slate-900 text-white rounded-xl font-black uppercase tracking-widest text-[9px] hover:bg-indigo-600 transition-colors flex items-center justify-center gap-1.5 mt-auto shadow-sm">
         {t('edit_profile')} <ArrowRight size={13} />
       </Link>
    </div>
  );
}
